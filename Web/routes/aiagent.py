...
aiagent_bp = Blueprint("aiagent", __name__, url_prefix="/aiagent")
...
from LLMAgent.stock_deep_research import StockDeepResearchAgent
...
def list_history():
    ...
    for f in reports_dir.iterdir():
        if f.is_file() and f.name.endswith(".md"):
            # Try parse Name_Code_Timestamp
            # Format: Stock_Deep_Research_{code}_{name}_{timestamp}.md
            # Example: Stock_Deep_Research_000001_平安银行_20231027_100000.md
            ...
            try:
                # Stock_Deep_Research_CODE_NAME_TIME...
                if len(parts) >= 4 and parts[0] == "Stock" and parts[1] == "Deep":
                     code = parts[3]
                     name = parts[4]
                     # timestamp might be parts[5] + parts[6] if present
                     if len(parts) >= 6:
                         # 20231027_100000
                         t_str = parts[5]
                         if len(parts) > 6:
...
def stock_preprocess_api():
    ...
    try:
        ...
        use_rag = bool(body.get("use_rag", True)) # Default True
