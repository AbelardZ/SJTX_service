...
│   ├── auto-app.js                # 前端交互逻辑（ECharts 图表、表格排序搜索）
│   ├── styles.css                 # 前端样式
│   └── data/                      # 基金季报/半年报 enriched CSV 数据
...
**新增公募基金重仓股跟踪模块 (fund_monitor_module)：**
- 基于公募基金定期报告披露数据，按申万行业层级追踪分析重仓股
- 支持行业排名图表、行业明细表、全市场 Top 300 重仓股
- 多维度排序（持仓市值、基金覆盖数、持仓占比）与搜索筛选
